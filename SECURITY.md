...
| Quantum break of secp256k1 | pre-committed Lamport key per address; consensus switch `pq_emergency_height` |
...
| Peer sending garbage, floods, huge messages | ban score, 24 h IP ban, token-bucket rate limit, 5 MB line cap |
...
| Memory exhaustion | capped mempool with fee-rate eviction, capped orphans and signature cache |
| Poisoning a valid block with a corrupted proof | PoW/structure failures are never cached as "invalid block" |
| Crash during write | checksummed block records; torn tail detected and truncated on startup |
...
- **Single implementation, in Python.** Every block is held in memory and
  replayed at startup; there is no UTXO database. Adequate for testnet, not
  for years of mainnet history.
...
- **No soft-fork activation logic yet**, so the quantum switch cannot be
  flipped by the network in a coordinated way today.
- **P2P gaps:** no headers-first sync, no compact blocks, no Tor/I2P, JSON
  wire format (hex doubles block size on the wire).
